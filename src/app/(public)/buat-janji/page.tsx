...
import { Check, ChevronLeft, ChevronRight, Lock, ArrowRight, ShieldCheck, Clock, Calendar, Stethoscope, CheckCircle2 } from 'lucide-react';
    ...
    return (
      <div className="max-w-lg mx-auto px-4 py-16 text-center">
        ...
        <div className="bg-[var(--color-surface)] rounded-xl p-6 mb-6 text-left">
          ...
          <p className="font-mono font-bold text-[var(--color-primary)]">{appointmentId}</p>
        </div>
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
          ...
          {STEPS.map((step, i) => (
            <div key={step.num} className="flex items-center">
              <div className={`flex items-center justify-center w-8 h-8 rounded-full text-sm font-medium ${
                currentStep > step.num ? 'bg-[var(--color-success)] text-white' :
                currentStep === step.num ? 'bg-[var(--color-primary)] text-white' :
                'bg-[var(--color-surface)] text-[var(--color-text-secondary)]'
              }`}>
                {currentStep > step.num ? <Check className="h-4 w-4" /> : step.num}
              </div>
              <span className="ml-2 text-xs text-[var(--color-text-secondary)]">{step.label}</span>
              {i < STEPS.length - 1 && <div className="w-8 h-px bg-[var(--color-border)] mx-2" />}
            </div>
          ))}
        </div>
        <div className="md:hidden">
          <p className="text-sm font-medium">Langkah {currentStep} dari {STEPS.length}</p>
          <div className="mt-2 h-2 bg-[var(--color-surface)] rounded-full">
            <div className="h-full bg-[var(--color-primary)] rounded-full transition-all" style={{ width: `${(currentStep / STEPS.length) * 100}%` }} />
          </div>
          <p className="mt-1 text-xs text-[var(--color-text-secondary)]">{STEPS[currentStep - 1].label}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-[var(--color-border)] p-6">
        ...
        {currentStep === 2 && (
          <div>
            <h2 className="text-lg font-semibold mb-4">Pilih Dokter</h2>
            {loading ? (
              ...
              <p className="text-[var(--color-text-secondary)] text-center py-8">Belum ada dokter tersedia untuk layanan ini.</p>
            ) : (
              <div className="space-y-3">
                {doctors.map((doctor) => (
                  <button
                  ...
                  >
                    ...
                    <p className="text-sm text-[var(--color-text-secondary)]">{doctor.specializationName} · {doctor.location}</p>
                  </button>
                ))}
              </div>
            )}
            ...
            {selectedDoctor && (
              <div className="p-3.5 bg-teal-50/80 border border-teal-200 rounded-xl text-xs space-y-1.5 shadow-2xs">
                ...
                <p className="text-gray-700 font-medium">
                  Hari Praktik Rutin: <span className="text-teal-900 font-bold">{selectedDoctor.schedules.map(s => `${s.day} (${s.startTime} - ${s.endTime})`).join(', ')}</span>
                </p>
                <p className="text-[11px] text-gray-500">
                  Lokasi Poliklinik: {selectedDoctor.location} &bull; Konsultasi di luar jadwal reguler akan dilayani pada sesi poliklinik terpadu spesialis.
                </p>
              </div>
            )}
        ...
        {currentStep === 5 && (
          <div>
            ...
            <div className="space-y-4">
              <Input label="Nama Lengkap" placeholder="Masukkan nama lengkap" required value={form.fullName} onChange={(e) => setForm({...form, fullName: e.target.value})} error={errors.fullName} />
              <Input label="Email" type="email" placeholder="Masukkan email" required value={form.email} onChange={(e) => setForm({...form, email: e.target.value})} error={errors.email} />
              <Input label="No. Telepon" placeholder="Masukkan nomor telepon" required value={form.phoneNumber} onChange={(e) => setForm({...form, phoneNumber: e.target.value})} error={errors.phoneNumber} />
              <Input label="Tanggal Lahir" type="date" value={form.birthDate} onChange={(e) => setForm({...form, birthDate: e.target.value})} />
              <Textarea label="Alamat" placeholder="Masukkan alamat" rows={2} value={form.address} onChange={(e) => setForm({...form, address: e.target.value})} />
              <Textarea label="Keluhan Singkat" placeholder="Bukan untuk diagnosis — hanya membantu dokter mempersiapkan konsultasi." rows={3} value={form.complaint} onChange={(e) => setForm({...form, complaint: e.target.value})} />
            </div>
          </div>
        )}
        ...
        {currentStep === 6 && (
          <div>
            <h2 className="text-lg font-semibold mb-4">Ringkasan Appointment</h2>
            <div className="space-y-4">
              <div className="bg-[var(--color-surface)] rounded-xl p-4">
                <p className="text-sm text-[var(--color-text-secondary)]">Layanan</p>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-[var(--color-surface)] rounded-xl p-4">
                  <p className="text-sm text-[var(--color-text-secondary)]">Tanggal</p>
                  <p className="font-medium">{selectedDate}</p>
                </div>
                <div className="bg-[var(--color-surface)] rounded-xl p-4">
                  <p className="text-sm text-[var(--color-text-secondary)]">Jam</p>
                  <p className="font-medium">{selectedTime}</p>
                </div>
              </div>
              <div className="bg-[var(--color-surface)] rounded-xl p-4">
                <p className="text-sm text-[var(--color-text-secondary)]">Data Pasien</p>
                <p className="font-medium">{form.fullName}</p>
                <p className="text-sm text-[var(--color-text-secondary)]">{form.email} · {form.phoneNumber}</p>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="flex items-center justify-between gap-3 mt-6">
        ...
        >
          {currentStep === 6 ? 'Konfirmasi Appointment' : 'Lanjutkan'}
